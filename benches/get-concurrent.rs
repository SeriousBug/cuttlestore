...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let couchdb = runtime.block_on(async {
        let store: Cuttlestore<String> =
            Cuttlestore::new("couchdb://admin:password@127.0.0.1:5984/cuttlestore_bench_get_conc")
                .await
                .unwrap();
        load(store.clone(), count_entries).await;
        store
    ...
    let surrealdb = runtime.block_on(async {
        let store: Cuttlestore<String> =
            Cuttlestore::new("surrealdb://root:root@127.0.0.1:8001/cuttlestore/bench_get_conc")
                .await
                .unwrap();
        load(store.clone(), count_entries).await;
        store
